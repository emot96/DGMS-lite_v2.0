function genPDF(){
    var doc = new jsPDF('p', 'pt', 'a4', true);
    var imgData='data:image/jpeg;base64,';
    var res = doc.autoTableHtmlToJson(document.getElementById("table1"));
    var res1 = doc.autoTableHtmlToJson(document.getElementById("table2"));
    var res2 = doc.autoTableHtmlToJson(document.getElementById("table3"));
    var res3 = doc.autoTableHtmlToJson(document.getElementById("table4"));
    var res4 = doc.autoTableHtmlToJson(document.getElementById("table5"));




    doc.addImage(imgData, 'jpeg', 270, 10, 62, 72); 
    doc.fromHTML($("#pdfName").get(0), 250, 85, {'width': 250});
    doc.fromHTML($("#customerName").get(0), 20, 130, {'width': 180});
    doc.fromHTML($("#assetDetail").get(0), 20, 180, {'width': 180});
    doc.fromHTML($("#assetAddress").get(0), 380, 180, {'width': 250});
    
  
    /*--------------------------------------------------table1------------------------------------------------------*/
    var options1 = {
        // beforePageContent: header,
        margin: {
        left:20,
        right:20,
        top: 10
        },
        theme:'grid',
        styles: {
                overflow: 'linebreak',
                fontSize: 10,
                valign: 'middle',
                // fillColor: [255, 193, 51]
            }, 
        showHead: 'firstPage', 
        
        didParseCell: function (data) {
            
            if (data.row.index ===  2 || data.row.index ===  7 || data.row.index ===  11 || data.row.index ===  16 || data.row.index ===  21 || data.row.index ===  26) {
                data.cell.styles.fillColor = [231, 232, 235];
                data.cell.styles.fontStyle = "bold";
                data.cell.styles.textColor = [139,0,139];
                data.cell.styles.halign = "left";
            }
        },
      
        startY: doc.autoTableEndPosY() + 260
        }; 


    /*--------------------------------------------------table2------------------------------------------------------*/
    var options2 = {
        // beforePageContent: header,
        margin: {
        left:20,
        right:20,
        top: 10
        },
        theme:'grid',
        styles: {
                overflow: 'linebreak',
                fontSize: 10,
                valign: 'middle',
                // fillColor: [255, 193, 51]
            },        
      
        startY: doc.autoTableEndPosY() + 200
        }; 

    /*--------------------------------------------------table3------------------------------------------------------*/
    var options3 = {
        // beforePageContent: header,
        margin: {
        left:20,
        right:20,
        top: 10
        },
        theme:'grid',
        styles: {
                overflow: 'linebreak',
                fontSize: 10,
                valign: 'middle',
                // fillColor: [255, 193, 51]
            },
        didParseCell: function (data) {
                if (data.row.index ===  1 || data.row.index ===  5 || data.row.index ===  10 ) {
                    data.cell.styles.fillColor = [231, 232, 235];
                    data.cell.styles.fontStyle = "bold";
                    data.cell.styles.textColor = [139,0,139];
                    data.cell.styles.halign = "left";
                }
            },            

      
        startY: doc.autoTableEndPosY() + 390
        }; 

        
   /*--------------------------------------------------table4------------------------------------------------------*/
   var options4 = {
    // beforePageContent: header,
    margin: {
    left:20,
    right:20,
    top: 10
    },
    theme:'grid',
    styles: {
            overflow: 'linebreak',
            fontSize: 10,
            valign: 'middle',
            // fillColor: [255, 193, 51]
        },        
  
    startY: doc.autoTableEndPosY() + 50
    }; 

    /*--------------------------------------------------table5------------------------------------------------------*/
   var options5 = {
    // beforePageContent: header,
    margin: {
    left:20,
    right:20,
    top: 10
    },
    theme:'grid',
    styles: {
            overflow: 'linebreak',
            fontSize: 10,
            valign: 'middle',
            // fillColor: [255, 193, 51]
        },        
  
    startY: doc.autoTableEndPosY() + 200
    }; 

  
    doc.autoTable(res.columns, res.data, options1);
    doc.autoTable(res1.columns, res1.data, options2);
    doc.autoTable(res2.columns, res2.data, options3);
    doc.addPage();
    doc.autoTable(res3.columns, res3.data, options4);
    doc.autoTable(res4.columns, res4.data, options5);
    




    const pageCount = doc.internal.getNumberOfPages();

    // For each page, print the page number and the total pages
    for(var i = 1; i <= pageCount; i++) {
     // Go to page i
    doc.setPage(i);
     //Print Page 1 of 4 for example
    doc.setFontSize(10)
    doc.text('visit us: https://www.emot.co.in/',20,830);
    doc.text('Page ' + String(i) + ' of ' + String(pageCount),577,830,null,null,"right");
    }


// Save the PDF
doc.save('asset details.pdf');
doc.output('datauri');

}